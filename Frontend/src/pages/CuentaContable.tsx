...
import { getCuentasContables, createCuentaContable, updateCuentaContable, deleteCuentaContable } from '@/api/cuentaContableApi';
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(formData); }} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <select id="tipoCuentaId" name="tipoCuentaId" value={formData.tipoCuentaId} onChange={handleChange} required className="w-full border rounded px-2 py-1">
      ...
      <div className="space-y-2">
        ...
        <Input type="number" id="nivel" name="nivel" value={formData.nivel} onChange={handleChange} required />
      </div>
      <div className="space-y-2">
      ...
      <div className="space-y-2">
        ...
        <select id="cuentaMayorId" name="cuentaMayorId" value={formData.cuentaMayorId} onChange={handleChange} className="w-full border rounded px-2 py-1">
      ...
      <div className="flex justify-end gap-3 pt-4">
        ...
        <Button type="submit">{formData.id !== 0 ? 'Actualizar Cuenta Contable' : 'Crear Cuenta Contable'}</Button>
      </div>
    </form>
  );
};
...
const CuentaContable = () => {
  ...
  const handleSubmit = async (data) => {
    if (data.id === 0) {
      ...
      setCuentasContables(cuentasContables.map(cc => (cc.id === updatedCuentaContable.id ? updatedCuentaContable : cc)));
    }
